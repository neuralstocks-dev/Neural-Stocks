...
- **Apr 23 — RapidAPI IDX integration (BASIC free tier · augment yfinance)**: added `services/idx_rapidapi.py` that wraps three endpoints from `indonesia-stock-exchange-idx` (yasimpratama88) on RapidAPI — `/api/emiten/{sym}/info` (quote snapshot), `/api/emiten/{sym}/key-stats` (valuation/profitability ratios), `/api/analysis/technical/{sym}` (SMA/EMA/RSI/MACD/Bollinger/Stochastic signals). For `.JK` tickers the analysis router now fetches these in parallel with yfinance; the RapidAPI values overlay the yfinance quote + fundamentals when present (richer: market cap in IDR, subsector, detailed ratios). yfinance is kept for OHLCV history (free, unlimited) and the RSS news scraper stays unchanged. **Budget protection**: BASIC plan is 1,000 req/month with **$0.01/req overage**, so we enforce a soft budget of **950 req/month** (~5% safety margin) via a MongoDB counter (`db.rapidapi_usage.{month: "YYYY-MM"}`). When exhausted, `_call()` returns `None` and the caller silently falls back to yfinance. Each IDX analysis consumes ~2-3 RapidAPI calls, so ~316 IDX analyses/month platform-wide at full consumption. **Per-tier daily caps** on IDX analyses (stricter than generic quotas to protect the shared monthly budget): Free 3/day, Pro 10/day, Elite 30/day, Day Pass 10/day — admins bypass. Enforced via new `enforce_idx_analysis_quota()` helper in `services/quota.py`. **Rate limiting**: 1 req/sec BASIC limit enforced in-process via asyncio Lock + 1.05s min interval. **UI**: subtle data-source chip on every IDX verdict header — green "IDX LIVE" when RapidAPI served the quote, grey "YF FALLBACK" when we fell back. New admin `IdxBudgetCard` on `/admin` shows month-to-date usage bar, remaining budget, soft-cap status chip (Healthy / Warn ≥80% / Exhausted / Not configured), last call timestamp + ticker, and step-by-step RapidAPI sign-up instructions when the key isn't set. New admin endpoint `GET /api/admin/rapidapi/usage`. **Env**: `RAPIDAPI_KEY` (empty by default), `RAPIDAPI_IDX_HOST`, `RAPIDAPI_IDX_MONTHLY_BUDGET=950`. **Caches**: quote 10m, technical 30m, fundamentals 24h — reduces duplicate calls for same-day repeat analyses. Verified end-to-end with no key set: `BBCA.JK` analysis returns `idx_data_source: "yfinance"`, admin card shows "Not configured" with onboarding steps, `usage_snapshot` returns `{configured: false, remaining: 950}`.
