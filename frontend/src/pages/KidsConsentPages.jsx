...
export function KidsAwaitingConsentPage() {
    ...
    const onResend = async () => {
        ...
        try {
            await axios.post(`${API_BASE}/kids/auth/resend-consent`, { email, lang });
            setResendOk(true);
        } catch (err) {
            setResendErr(err?.response?.data?.detail || (lang === "id" ? "Tidak bisa kirim ulang sekarang — coba lagi sebentar." : "Couldn't resend right now — try again in a minute."));
        } finally {
            setResending(false);
        }
    };
        ...
        <div style={shellStyle} data-testid="kids-awaiting-consent-page">
            ...
            <div style={cardStyle}>
                ...
                <h2 style={{ fontSize: 24, fontWeight: 700, textAlign: "center", marginBottom: 12 }}>
                    {t(lang, "consent.awaiting_title")}
                </h2>
                <p style={{ fontSize: 15, lineHeight: 1.65, textAlign: "center", opacity: 0.85 }}>
                    {t(lang, "consent.awaiting_body", { parent: parentEmail || (lang === "id" ? "orang tuamu" : "your parent") })}
                </p>
                <div style={{ background: "#fff8f0", border: "1.5px solid #e5d5b8", borderRadius: 14, padding: 16, marginTop: 20, fontSize: 13, lineHeight: 1.6 }}>
                    <p style={{ fontWeight: 700, marginBottom: 6 }}>{t(lang, "consent.awaiting_what_next")}</p>
                    <ol style={{ margin: 0, paddingLeft: 18 }}>
                        <li>{t(lang, "consent.awaiting_step1")}</li>
                        <li>{t(lang, "consent.awaiting_step2")}</li>
                        <li>{t(lang, "consent.awaiting_step3")}</li>
                    </ol>
                </div>

                <div style={{ marginTop: 24, padding: 16, borderRadius: 12, border: "1px dashed #e5d5b8" }}>
                    <p style={{ fontSize: 13, marginBottom: 12, fontWeight: 600 }}>{t(lang, "consent.awaiting_resend_label")}</p>
                    <button
                        onClick={onResend}
                    ...
                    >
                        ...
                        {resendOk ? t(lang, "consent.awaiting_resend_ok") : resending ? t(lang, "consent.awaiting_resend_sending") : t(lang, "consent.awaiting_resend_btn")}
                    </button>
                ...
                <p style={{ marginTop: 24, fontSize: 12, textAlign: "center", opacity: 0.6 }}>
                    {t(lang, "consent.awaiting_already")} <Link to="/kids/login" style={{ color: ACCENT, fontWeight: 700 }} data-testid="kids-go-login-from-awaiting">{t(lang, "consent.awaiting_login_link")}</Link>
                </p>
            </div>
        </div>
    );
...
export function KidsParentalConsentPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled) setLoadErr(err?.response?.data?.detail || (lang === "id" ? "Kami tidak bisa memuat link persetujuan ini." : "We couldn't load this consent link."));
            }
        })();
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            setSubmitErr(err?.response?.data?.detail || (lang === "id" ? "Tidak bisa mengirim persetujuanmu — coba lagi." : "Couldn't submit your consent — try again."));
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <div style={shellStyle} data-testid="kids-parent-consent-page">
                ...
                <div style={cardStyle}>
                    <div style={{ textAlign: "center" }}>
                        ...
                        <h2 style={{ fontSize: 22, fontWeight: 700, marginTop: 12 }}>{t(lang, "consent.parent_invalid_title")}</h2>
                        <p style={{ fontSize: 14, opacity: 0.8, marginTop: 8, lineHeight: 1.55 }}>{loadErr}</p>
                        <p style={{ fontSize: 13, marginTop: 18, opacity: 0.7 }}>{t(lang, "consent.parent_invalid_body")}</p>
                    </div>
                </div>
            </div>
        );
                ...
                <div style={cardStyle}>
                    <div style={{ textAlign: "center" }}>
                        ...
                        <h2 style={{ fontSize: 24, fontWeight: 700, marginTop: 12 }}>
                            {t(lang, "consent.parent_done_title")}
                        </h2>
                        <p style={{ fontSize: 15, opacity: 0.85, marginTop: 10, lineHeight: 1.6 }}>
                            {t(lang, "consent.parent_done_body", { kid: preview.kid_full_name })}{" "}
                            <Link to="/kids/login" style={{ color: ACCENT, fontWeight: 700 }}>kids/login</Link>.
                        </p>
                        <p style={{ fontSize: 13, marginTop: 18, opacity: 0.7, lineHeight: 1.55 }}>
                            {t(lang, "consent.parent_done_withdraw")}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div style={shellStyle} data-testid="kids-parent-consent-page">
            ...
            <div style={cardStyle}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                    ...
                    <h2 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>{t(lang, "consent.parent_title")}</h2>
                </div>
                <p style={{ fontSize: 14, lineHeight: 1.65, opacity: 0.85 }}>
                    {t(lang, "consent.parent_intro")}
                </p>

                <div style={{ background: "#fff8f0", border: `1.5px solid #e5d5b8`, borderRadius: 14, padding: 18, marginTop: 18 }} data-testid="kid-preview">
                    <p style={{ fontSize: 11, letterSpacing: 1.5, color: "#7a5a00", textTransform: "uppercase", margin: 0, fontWeight: 700 }}>{t(lang, "consent.parent_kid_label")}</p>
                    <p style={{ fontSize: 16, fontWeight: 700, marginTop: 8 }}>{preview.kid_full_name}</p>
                    <p style={{ fontSize: 13, opacity: 0.7, marginTop: 2 }}>
                        {preview.kid_email} · {lang === "id" ? "usia" : "age"} {preview.kid_age_at_signup}
                    </p>
                </div>

                <div style={{ marginTop: 22 }}>
                    <p style={{ fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#7a5a00", fontWeight: 700, marginBottom: 8 }}>{t(lang, "consent.parent_collect_label")}</p>
                    <ul style={{ margin: 0, paddingLeft: 18, fontSize: 14, lineHeight: 1.7 }}>
                        <li>{t(lang, "consent.parent_collect_1")}</li>
                        <li>{t(lang, "consent.parent_collect_2")}</li>
                        <li>{t(lang, "consent.parent_collect_3")}</li>
                    </ul>
                    <p style={{ fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#7a5a00", fontWeight: 700, marginTop: 18, marginBottom: 8 }}>{t(lang, "consent.parent_never_label")}</p>
                    <ul style={{ margin: 0, paddingLeft: 18, fontSize: 14, lineHeight: 1.7 }}>
                        <li>{t(lang, "consent.parent_never_1")}</li>
                        <li>{t(lang, "consent.parent_never_2")}</li>
                        <li>{t(lang, "consent.parent_never_3")}</li>
                        <li>{t(lang, "consent.parent_never_4")}</li>
                    </ul>
                </div>

                <form onSubmit={onSubmit} style={{ marginTop: 24 }} data-testid="kid-consent-form">
                    <label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 6 }}>
                        {t(lang, "consent.parent_name_label")} <span style={{ opacity: 0.6, fontWeight: 400 }}>{t(lang, "consent.parent_name_hint")}</span>
                    </label>
                    <input
                    ...
                    <label style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, lineHeight: 1.55, cursor: "pointer" }}>
                        ...
                        />
                        <span>{t(lang, "consent.parent_agree_text", { kid: preview.kid_full_name })}</span>
                    </label>
                    ...
                    >
                        {submitting ? <Loader2 size={16} className="animate-spin" /> : <><Heart size={16} /> {t(lang, "consent.parent_submit")}</>}
                    </button>
                </form>
            </div>
        </div>
