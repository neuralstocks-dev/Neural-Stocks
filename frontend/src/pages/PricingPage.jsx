    ...
    return (
        <>
            <div className="max-w-[1400px] mx-auto px-5 md:px-8 pt-10 pb-16" data-testid="pricing-page">
                ...
                {/* Payment provider trust badge — reassures buyers this is a real, secure checkout */}
                <div
                    className="mt-10 flex flex-wrap items-center justify-center gap-4 py-5"
                    style={{ borderTop: "1px solid hsl(var(--border-divider))", borderBottom: "1px solid hsl(var(--border-divider))" }}
                    data-testid="payment-trust-badge"
                >
                    ...
                    <img
                        ...
                        style={{ height: 20 }}
                    />
                    <span className="font-mono text-[10px]" style={{ color: "hsl(var(--text-muted))" }}>&amp;</span>
                    <StripeMark height={16} color="#635BFF" />
                </div>
    ...
    return (
        ...
        >
            {processing ? (
                ...
                <>
                    Pay with Card
                    <StripeMark height={13} color="#635BFF" />
                </>
            )}
        </button>
    );
...
// Stripe's actual wordmark (sourced from checkout.stripe.com's own rendering) — not a
// styled-text approximation, so it stays compliant with Stripe's brand guidelines.
function StripeMark({ height = 14, color = "#635BFF" }) {
    return (
        <svg
            width={(height * 33) / 15}
            height={height}
