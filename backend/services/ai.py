...
- `intrinsic_value_anchor.ev_multiples` now contains EV/EBITDA and EV/FCF multiples benchmarked against Damodaran sector medians (January 2026). When `ev_multiples.data_quality` is "full" or "partial": state the computed multiple, the sector benchmark, and the premium/discount percentage in `fundamental_analysis` prose (e.g. "Apple trades at 28x EV/EBITDA versus a Technology sector median of 24.5x — a 14% premium consistent with its margin profile"). When data_quality is "insufficient" or the sector is None/excluded (Financials), skip EV multiples entirely.
- `macro_context` contains live macroeconomic regime data (Fed Funds Rate, CPI YoY, 10Y yield). When present: incorporate `rate_sensitive_note` into `fundamental_analysis` for rate-sensitive sectors (Utilities, REITs, Financials, Consumer Discretionary); note the `rate_cycle` (hiking/cutting/holding) and its sector implications; for non-rate-sensitive sectors mention macro only if CPI or yields materially affect cost structure. If `macro_context` is absent, omit macro claims entirely. Do NOT treat a premium or discount as a directional signal on its own — always contextualise against the company's growth rate and margin profile.
"""
...
- `macro_context` contains live macroeconomic regime data (Fed Funds Rate, CPI YoY, 10Y yield). When present: incorporate `rate_sensitive_note` into `fundamental_analysis` for rate-sensitive sectors (Utilities, REITs, Financials, Consumer Discretionary); note the `rate_cycle` (hiking/cutting/holding) and its sector implications; for non-rate-sensitive sectors mention macro only if CPI or yields materially affect cost structure. If `macro_context` is absent, omit macro claims entirely.
